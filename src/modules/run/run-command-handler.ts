...
type Dependencies = Readonly<{
  ...
  consumeRateLimit(userId: string, runId: string, commandType: RunCommand["type"]): boolean;
}>;
...
export function createRunCommandHandler(dependencies: Dependencies) {
  return async (request: Request, runId: string): Promise<Response> => {
    ...
    try {
      ...
      if (!dependencies.consumeRateLimit(identity.userId, runId, command.type)) {
      ...
      if (/sequence|version conflict|phase|room|promotion|combat|intervention/i.test(message)) {
